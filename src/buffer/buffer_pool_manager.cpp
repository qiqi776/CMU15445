...
void BufferPoolManager::FlushAllPagesUnsafe() {
  ...
  for (const auto &[page_id, frame_id] : page_table_) {
    (void)page_id;
    FlushFrameUnsafe(frames_[frame_id]);
  }
}
...
void BufferPoolManager::FlushFrameUnsafe(const std::shared_ptr<FrameHeader> &frame) {
...
auto BufferPoolManager::AcquireFrame() -> std::optional<std::shared_ptr<FrameHeader>> {
  ...
  auto frame = frames_[victim_opt.value()];
  BUSTUB_ASSERT(frame->pin_count_.load() == 0, "evicted frame should not be pinned");
  BUSTUB_ASSERT(frame->page_id_.has_value(), "evicted frame should contain a valid page");
